...
#    Feb 18, 2020 07:58:47 PM IST  platform: Windows NT
...
def vp_start_gui():
    ...
    root = tk.Tk()
    top = Toplevel1 (root)
    delete_member_window_support.init(root, top)
    root.mainloop()
...
def create_Toplevel1(root, *args, **kwargs):
    ...
    w = tk.Toplevel (root)
    top = Toplevel1 (w)
    delete_member_window_support.init(w, top, *args, **kwargs)
    return (w, top)
...
class Toplevel1:
    ...
    def delete_member(self):
        self.delete_member1(self.Member_ID_Entry.get())
    def delete_member1(self,member_id):
        try:
            ...
            try:
                self.cursor.execute(
                    "declare nCount NUMBER; begin SELECT count(*) into nCount FROM user_tables where table_name = 'LMS_BOOK_DETAILS'; IF(nCount <= 0) THEN execute immediate ('Create table LMS_BOOK_DETAILS(BOOK_CODE Varchar2(10), BOOK_TITLE Varchar2(50) NOT NULL, CATEGORY Varchar2(15) NOT NULL, AUTHOR Varchar2(30) NOT NULL, PUBLICATION Varchar2(30), PUBLISH_DATE date, BOOK_EDITION int, PRICE number(8,2) NOT NULL, RACK_NUM Varchar2(3), DATE_ARRIVAL date NOT NULL, SUPPLIER_ID Varchar2(3) NOT NULL, Constraint LMS_cts4 PRIMARY KEY(BOOK_CODE), Constraint LMS_cts41  on delete cascade on delete cascade(SUPPLIER_ID) References LMS_SUPPLIERS_DETAILS(SUPPLIER_ID) on delete cascade on delete cascade)'); END IF; end;")
            except Exception as e:
                messagebox.showerror("bookdetails table", e)
            try:
                self.cursor.execute(
                    "declare nCount NUMBER; begin SELECT count(*) into nCount FROM user_tables where table_name = 'LMS_BOOK_ISSUE'; IF(nCount <= 0) THEN execute immediate ('Create table LMS_BOOK_ISSUE(BOOK_ISSUE_NO int,MEMBER_ID Varchar2(10) NOT NULL, BOOK_CODE Varchar2(10) NOT NULL, DATE_ISSUE date NOT NULL, DATE_RETURN date NOT NULL, DATE_RETURNED date NULL,BOOK_ISSUE_STATUS varchar2(20), FINE_RANGE Varchar2(3), Constraint LMS_cts5 PRIMARY KEY(BOOK_ISSUE_NO), Constraint LMS_Mem FOREIGN KEY(MEMBER_ID) References LMS_MEMBERS(MEMBER_ID) on delete cascade on delete cascade, Constraint LMS_BookDetail FOREIGN KEY(BOOK_CODE) References LMS_BOOK_DETAILS(BOOK_CODE) on delete cascade on delete cascade, Constraint LMS_FineDetail FOREIGN KEY(FINE_RANGE) References LMS_FINE_DETAILS(FINE_RANGE) on delete cascade)'); END IF; end;")
            except Exception as e:
        ...
        try:
            statement='delete from LMS_MEMBERS where MEMBER_ID=:1'
            self.cursor.execute(statement,{'1':member_id})
            self.conn.commit()
    ...
    def __init__(self, Delete_Member_Window=None):
        try:
            self.conn = cx_Oracle.connect('HR/root')
            self.cursor = self.conn.cursor()
            try:
            ...
            try:
                self.cursor.execute(
                    "declare nCount NUMBER; begin SELECT count(*) into nCount FROM user_tables where table_name = 'LMS_BOOK_DETAILS'; IF(nCount <= 0) THEN execute immediate ('Create table LMS_BOOK_DETAILS(BOOK_CODE Varchar2(10), BOOK_TITLE Varchar2(50) NOT NULL, CATEGORY Varchar2(15) NOT NULL, AUTHOR Varchar2(30) NOT NULL, PUBLICATION Varchar2(30), PUBLISH_DATE date, BOOK_EDITION int, PRICE number(8,2) NOT NULL, RACK_NUM Varchar2(3), DATE_ARRIVAL date NOT NULL, SUPPLIER_ID Varchar2(3) NOT NULL, Constraint LMS_cts4 PRIMARY KEY(BOOK_CODE), Constraint LMS_cts41  on delete cascade(SUPPLIER_ID) References LMS_SUPPLIERS_DETAILS(SUPPLIER_ID) on delete cascade)'); END IF; end;")
            except Exception as e:
                messagebox.showerror("bookdetails table", e)
            try:
                self.cursor.execute(
        ...
        Delete_Member_Window.geometry("1232x741+310+64")
        Delete_Member_Window.minsize(148, 1)
        Delete_Member_Window.maxsize(4112, 1055)
        Delete_Member_Window.resizable(0, 0)
        Delete_Member_Window.title("DELETE MEMBER WINDOW ")
        Delete_Member_Window.configure(background="#b42cc7")

        self.Delete_Member_Window_Frame = tk.Frame(Delete_Member_Window)
        self.Delete_Member_Window_Frame.place(relx=0.049, rely=0.063, relheight=0.884
                                              , relwidth=0.916)
        self.Delete_Member_Window_Frame.configure(relief='groove')
        self.Delete_Member_Window_Frame.configure(borderwidth="2")
        self.Delete_Member_Window_Frame.configure(relief="groove")
        self.Delete_Member_Window_Frame.configure(background="#5f86d8")

        self.Private_Label = tk.Label(self.Delete_Member_Window_Frame)
        ...
        self.Date_Label = tk.Label(self.Delete_Member_Window_Frame)
        ...
        self.Datenow_Label = tk.Label(self.Delete_Member_Window_Frame)
        ...
        self.Time_LAbel = tk.Label(self.Delete_Member_Window_Frame)
        ...
        self.Timenow_Label = tk.Label(self.Delete_Member_Window_Frame)
        ...
        self.Member_ID_Label = tk.Label(self.Delete_Member_Window_Frame)
        ...
        self.Member_ID_Entry = tk.Entry(self.Delete_Member_Window_Frame)
        ...
        self.Delete_Button = tk.Button(self.Delete_Member_Window_Frame)
        ...
        self.Delete_Button.configure(command=self.delete_member, text='''DELETE''')

        self.menubar = tk.Menu(Delete_Member_Window, font=font9, bg=_bgcolor, fg=_fgcolor)
        Delete_Member_Window.configure(menu=self.menubar)

        self.sub_menu = tk.Menu(Delete_Member_Window, tearoff=0)
        ...
        self.sub_menu.add_command(
            ...
            foreground="#000000",
            label="Exit",
            command=delete_member_window_support.destroy_window)
